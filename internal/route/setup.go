...
func SetUpRouter(employeesController *controller.EmployeesController, positionsController *controller.PositionsController, env *internal.Env, mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /positions/{id}", middleware.Adapt(middleware.JwtMiddleware(positionsController.GetPosition, env.JWTTokenSecret), middleware.Logger()))
	mux.HandleFunc("POST /positions", middleware.Adapt(middleware.JwtMiddleware(positionsController.CreatePosition, env.JWTTokenSecret), middleware.Logger()))
	mux.HandleFunc("DELETE /positions/{id}", middleware.Adapt(middleware.JwtMiddleware(positionsController.DeletePosition, env.JWTTokenSecret), middleware.Logger()))
	mux.HandleFunc("PUT /positions/{id}", middleware.Adapt(middleware.JwtMiddleware(positionsController.UpdatePosition, env.JWTTokenSecret), middleware.Logger()))
	mux.HandleFunc("GET /positions", middleware.Adapt(middleware.JwtMiddleware(positionsController.GetAllPositions, env.JWTTokenSecret), middleware.Logger()))
